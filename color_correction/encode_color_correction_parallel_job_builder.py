...
def main():
    ...
    for batch in range(0, batches):
        ...
        for job in range(job_num, job_num + 1000):
            if job == len(images):
                break
            if "z1500" in images[job]:
                matrix = config["z1500"]
            elif "z2500" in images[job]:
                matrix = config["z2500"]
            else:
                raise ValueError("Unknown zoom level in image {0}".format(images[job]))
            file_parts = images[job].split('/')
            batchfile.write("job_args = " + images[job] + " " + matrix + " " + os.path.join(args.outdir, file_parts[-2],
                                                                                            file_parts[-1]) + "\n")
